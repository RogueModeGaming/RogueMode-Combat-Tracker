...
{
    // Combat Tracker v0.9.0.3 Beta: aggregated status-damage attribution release.
    private const string TrackerVersion = "v0.9.0.3 Beta";
    ...
    private const double MountedPalSkillGraceSeconds = 5.0;
    private const double MountedPalSkillMaximumActiveSeconds = 15.0;
    ...
    // Q telemetry can include the exact player rider for mounted Pal Waza
    // actions. Keep the latest action per rider so player-routed mounted-skill
    // hits can be reassigned after the C correlation proves there was no
    // confirmed player weapon source.
    private readonly Dictionary<string, MountedPalSkillContext>
        _mountedPalSkillByRider = new(StringComparer.Ordinal);
    ...
    // An inactivity timeout is a provisional safety stop, not a hard end.
    // If the exact same target actor takes friendly damage again, continue the
    // existing encounter while excluding the idle gap from combat time.
    private bool _encounterResumableAfterInactivity;
    private EncounterSnapshot? _provisionalInactivitySnapshot;
    ...
    // Live target HP is supplied by Lua H telemetry for the exact active
    // target actor. Numeric HP may be exact or rate-derived depending on the
    // reflected Palworld getter return type, while HPRate is authoritative for
    // the progress bar.
    private string? _targetHpActorId;
    private double? _targetCurrentHp;
    private double? _targetMaxHp;
    private double? _targetHpRate;
    private string _targetHpQuality = "Unavailable";
    ...
    // P|ACTIVE is emitted for deployed Pals visible to this client, including
    // remote players' Pals. Keep those signals pending until O ownership
    // proves that the Pal belongs to the local player.
    private readonly Dictionary<string, string> _activePalCandidates =
        new(StringComparer.Ordinal);

    // Emitted directly by Lua through PalUtility.GetPlayerCharacter. L is the
    // authoritative local-player identity; ownership and Pal-state events must
    // never replace it with a remote player's actor.
    ...
    private void TryAttachTelemetry()
    {
        ...
        try
        {
            ...
            InspectExistingTelemetryBootstrap(telemetryPath);
    ...
    private void ProcessTelemetryLine(string line)
    {
                ...
                out int actionSequence))
        {
            ProcessPalActionEvent(
                ...
                fields[12],
                fields[13],
                fields.Length >= 15 ? fields[14] : string.Empty,
                fields.Length >= 16 ? fields[15] : string.Empty
            );
            return;
        }
        ...
        if (fields.Length >= 15 &&
            ...
            double.TryParse(
                ...
                NumberStyles.Float,
                CultureInfo.InvariantCulture,
                out double correlatedSourceAge))
        {
        ...
        // Aggregated Burn/Poison damage delta (not one line per tick):
        // T|time|sequence|defender|actual|raw|status ID|status name|
        // source type|source actor|source name|source label|identity|...|
        // match method|active count|active names|generation|tick count|flush reason
        if (fields.Length >= 23 &&
        ...
        // Live target HP:
        // H|timestamp|target actor|current HP|max HP|HP rate|quality
        if (fields.Length >= 7 &&
            fields[0].Equals("H", StringComparison.OrdinalIgnoreCase) &&
            TryParseTimestamp(fields[1], out _))
        {
            ProcessTargetHpEvent(
                fields[2],
                fields[3],
                fields[4],
                fields[5],
    ...
    private void ProcessTargetHpEvent(
        ...
        string quality)
    {
        ...
        bool hasCurrentHp = double.TryParse(
            currentHpText,
            NumberStyles.Float,
            CultureInfo.InvariantCulture,
            out double currentHp) &&
            !double.IsNaN(currentHp) &&
            !double.IsInfinity(currentHp) &&
            currentHp >= 0;

        bool hasMaxHp = double.TryParse(
            maxHpText,
            NumberStyles.Float,
            CultureInfo.InvariantCulture,
            out double maxHp) &&
            !double.IsNaN(maxHp) &&
            !double.IsInfinity(maxHp) &&
            maxHp > 0;

        bool hasHpRate = double.TryParse(
            hpRateText,
            NumberStyles.Float,
            CultureInfo.InvariantCulture,
            out double hpRate) &&
            !double.IsNaN(hpRate) &&
            !double.IsInfinity(hpRate);

        if (hasHpRate)
        {
            hpRate = Math.Clamp(hpRate, 0, 1);
        }

        // Keep the parser tolerant of future Lua refinements: if two of the
        // three values are available, recover the third when it is safe.
        if (!hasCurrentHp && hasMaxHp && hasHpRate)
        {
            currentHp = maxHp * hpRate;
            hasCurrentHp = true;
        }
        else if (!hasMaxHp && hasCurrentHp && hasHpRate && hpRate > 0.000001)
        {
            maxHp = currentHp / hpRate;
            hasMaxHp = maxHp > 0;
        }
        else if (!hasHpRate && hasCurrentHp && hasMaxHp)
        {
            hpRate = Math.Clamp(currentHp / maxHp, 0, 1);
            hasHpRate = true;
        }
        ...
        _targetHpActorId = targetActorId;
        _targetCurrentHp = hasCurrentHp ? currentHp : null;
        _targetMaxHp = hasMaxHp ? maxHp : null;
        _targetHpRate = hasHpRate ? hpRate : null;
        _targetHpQuality = string.IsNullOrWhiteSpace(quality)
            ? "Unavailable"
            : quality.Trim().ToUpperInvariant();
        ...
        string? sourceName)
    {
        ...
        else
        {
            ...
            {
                // Safety Stop is resumable only when this is the exact same
                // target actor and the new hit is from a tracked friendly
                // source. Confirmed death/manual stop remain hard endings.
                if (!(attackerIsPlayer || attackerIsPal) ||
                    !TryResumeInactivityEncounter(defender, nowSeconds))
                {
                    return;
                }
            }
        ...
        {
            // Deployed Pals from both the local and remote players use PAL.
            // Never infer ownership from SourceType alone; O telemetry is the
            // authoritative owner mapping.
            _palDamage += damage;

            if (attackerIsActivePal &&
                IsPalOwnedByLocalPlayer(attacker))
            {
        ...
        string riderDisplayName)
    {
        ...
        RemoveExpiredMountedPalSkillContexts(telemetryTimestamp);

        TrackMountedPalSkillContext(
            telemetryTimestamp,
            phase,
            actor,
            sourceType,
    ...
    private void TrackMountedPalSkillContext(
        double telemetryTimestamp,
        string phase,
        string palActor,
        string sourceType,
    ...
    {
        if (!IsPalSourceType(sourceType) ||
            !IsMeaningfulTelemetryValue(palActor) ||
            !IsMeaningfulPalSkillName(skillName) ||
            !IsPlayerActor(riderActor))
        {
            return;
        }

        string normalizedPalName = NormalizeSourceName(palName);
        string normalizedSkillName = NormalizePalSkillLabel(skillName);
        string resolvedRiderName = ResolveStablePlayerDisplayName(
            riderActor,
            riderDisplayName
        );

        CacheCombatantName(palActor, normalizedPalName);
        CacheCombatantName(riderActor, resolvedRiderName, priority: 3);
        AssignPalOwner(
            palActor,
            riderActor,
            resolvedRiderName
        ...
        if (phase.Equals("BEGIN", StringComparison.OrdinalIgnoreCase))
        {
            _mountedPalSkillByRider[riderActor] =
                new MountedPalSkillContext(
                    riderActor,
                    resolvedRiderName,
                    palActor,
                    normalizedPalName,
        ...
        if (!_mountedPalSkillByRider.TryGetValue(
                riderActor,
                out MountedPalSkillContext? context))
        {
            return;
        }

        bool samePal = context.PalActorId.Equals(
            palActor,
            StringComparison.Ordinal);
        bool sameSkill =
            context.SkillName.Equals(
                normalizedSkillName,
                StringComparison.OrdinalIgnoreCase) ||
            (IsMeaningfulTelemetryValue(skillId) &&
             context.SkillId.Equals(
                 skillId,
                 StringComparison.OrdinalIgnoreCase));

        if (samePal && sameSkill &&
            (phase.Equals("END", StringComparison.OrdinalIgnoreCase) ||
             phase.Equals("BREAK", StringComparison.OrdinalIgnoreCase)))
        {
            context.EndTelemetryTimestamp = telemetryTimestamp;
            context.EndPhase = phase;
        }
    }

    private void RemoveExpiredMountedPalSkillContexts(
        double currentTelemetryTimestamp)
    {
        string[] expired = _mountedPalSkillByRider
            .Where(pair =>
            {
                MountedPalSkillContext context = pair.Value;
                double age = context.EndTelemetryTimestamp.HasValue
                    ? currentTelemetryTimestamp -
                      context.EndTelemetryTimestamp.Value
                    : currentTelemetryTimestamp -
                      context.StartTelemetryTimestamp;

                double maximumAge = context.EndTelemetryTimestamp.HasValue
                    ? MountedPalSkillGraceSeconds
                    : MountedPalSkillMaximumActiveSeconds;
        ...
        foreach (string riderActor in expired)
        {
            _mountedPalSkillByRider.Remove(riderActor);
        }
    }

    private static bool IsConfirmedPlayerCorrelation(
        string sourceLabel,
        string state,
        double sourceAgeSeconds)
    {
        if (!IsMeaningfulTelemetryValue(sourceLabel))
        {
            return false;
        }
        ...
        bool confirmedWeaponEvent =
            state.Equals("SHOT", StringComparison.OrdinalIgnoreCase) ||
            state.Equals("BOW_RELEASE", StringComparison.OrdinalIgnoreCase) ||
            state.Equals("TRIGGER", StringComparison.OrdinalIgnoreCase);

        return confirmedWeaponEvent &&
            sourceAgeSeconds >= -0.01 &&
            sourceAgeSeconds <= 3.0;
    }

    private bool TryApplyMountedPalSkillAttribution(
        double telemetryTimestamp,
        string sourceType,
        string riderActor,
        string sourceLabel,
        ...
        PendingDamageMetadataMatch match)
    {
        if (!sourceType.Equals(
                "PLAYER",
                StringComparison.OrdinalIgnoreCase) ||
            match.HasAttributionOverride ||
            match.AggregateReassigned ||
            IsConfirmedPlayerCorrelation(
                sourceLabel,
                state,
                sourceAgeSeconds))
        {
        ...
        if (telemetryTimestamp < context.StartTelemetryTimestamp - 0.25)
        {
            return false;
        }

        if (context.EndTelemetryTimestamp.HasValue &&
            telemetryTimestamp - context.EndTelemetryTimestamp.Value >
                MountedPalSkillGraceSeconds)
        {
            return false;
        }
        ...
        match.AttributedActor = context.PalActorId;
        match.AttributedSourceType = "PAL";
        match.AttributedSourceName = context.PalName;
        match.ExactSourceLabel = context.SkillName;
        match.AggregateReassigned = true;

        CacheCombatantName(context.PalActorId, context.PalName);
        AssignPalOwner(
            context.PalActorId,
            context.RiderActorId,
            context.RiderDisplayName
        );
        ...
        PalSkillRuntimeEntry skill = RegisterPalSkill(
            context.PalActorId,
            "PAL",
            context.PalName,
            context.SkillId,
        ...
        int bodyPartType)
    {
        ...
        if (TryApplyMountedPalSkillAttribution(
                telemetryTimestamp,
                sourceType,
                attacker,
                sourceLabel,
                ...
                match))
        {
            FinalizePendingDamageSource(
                match,
                match.ExactSourceLabel ?? "Mounted Pal Skill",
                bodyPartType
            );
            return;
        }
        ...
        string classification)
    {
        ...
        // B telemetry explicitly identifies the player whose partner Pal
        // produced this bonus. Persist that ownership before moving the
        // aggregate damage so encounter grouping cannot fall back to the
        // local player.
        string resolvedPlayerName = NormalizeSourceName(playerName);

        if (string.IsNullOrWhiteSpace(resolvedPlayerName))
        {
            resolvedPlayerName = GetFriendlyActorName(playerActor);
        }
        ...
        int basePower)
    {
        ...
        // DamageResult.WeaponType is not a reliable weapon identity for Pal
        // damage. Mounted Pal skills can arrive with the same internal value
        // used by firearms (for example 3), so Pal fallback labels must be
        // skill/element based rather than weapon-table based.
        if (isPal)
        {
    ...
    private string ResolveStablePlayerDisplayName(
        string playerActor,
        string? candidateDisplayName)
    {
        string candidate = NormalizeSourceName(candidateDisplayName);

        if (!string.IsNullOrWhiteSpace(candidate))
        {
            candidate = GetFriendlyActorName(candidate);
        }

        if (IsGenericCombatantDisplayName(candidate) &&
            _knownCombatantNames.TryGetValue(
                playerActor,
                out string? knownName) &&
            !IsGenericCombatantDisplayName(knownName))
        {
            return knownName;
        }
        ...
        if (_knownCombatantNames.TryGetValue(
                playerActor,
                out string? fallbackKnownName))
        {
            return fallbackKnownName;
        }

        return GetFriendlyActorName(playerActor);
    }
        ...
        string playerDisplayName)
    {
        ...
        string resolvedPlayerName =
            ResolveStablePlayerDisplayName(
                playerActor,
                playerDisplayName
            );
        ...
        string ownerDisplayName)
    {
        ...
        // P|ACTIVE can describe a remote player's Pal. Only promote the
        // pending active-state signal after O proves this Pal belongs to the
        // player identified by authoritative L telemetry.
        if (_activePalCandidates.TryGetValue(
                palActor,
                out string? candidateDisplayName) &&
            IsLocalPlayerActor(ownerActor))
        {
            SetActivePal(
                palActor,
        ...
        string ownerDisplayName)
    {
        ...
        PromotePendingLocalActivePal();
    }

    private bool IsLocalPlayerActor(string? actor)
    {
        return !string.IsNullOrWhiteSpace(actor) &&
            !string.IsNullOrWhiteSpace(_localPlayerActorId) &&
            string.Equals(
                actor,
                _localPlayerActorId,
                StringComparison.Ordinal);
    ...
    private bool IsPalOwnedByLocalPlayer(string palActor)
    {
        return _palOwners.TryGetValue(
                   palActor,
                   out PalOwnerInfo? ownerInfo) &&
               IsLocalPlayerActor(ownerInfo.OwnerActorId);
    }

    private void PromotePendingLocalActivePal()
    {
        ...
        foreach (KeyValuePair<string, string> candidate in
                 _activePalCandidates.Reverse())
        {
            if (!IsPalOwnedByLocalPlayer(candidate.Key))
            {
                continue;
            }
        ...
        string displayName)
    {
        ...
        if (state.Equals(
                ...
                StringComparison.OrdinalIgnoreCase))
        {
            _activePalCandidates.Remove(actor);
            _activePalCandidates[actor] = displayName;

            if (IsPalOwnedByLocalPlayer(actor))
            {
    ...
    {
        // A P record alone is not local-player proof. O must already map the
        // Pal to the actor identified by L telemetry.
        if (!IsPalOwnedByLocalPlayer(actor))
        {
            return;
        }
    ...
    private static bool IsGenericCombatantDisplayName(string? displayName)
    {
        ...
        return value.Equals("Player", StringComparison.OrdinalIgnoreCase) ||
            value.Equals("Player Female", StringComparison.OrdinalIgnoreCase) ||
            value.Equals("Player Male", StringComparison.OrdinalIgnoreCase) ||
            value.StartsWith("BP_Player_", StringComparison.OrdinalIgnoreCase) ||
            value.Equals("Pal", StringComparison.OrdinalIgnoreCase) ||
        ...
        int priority = 1)
    {
        ...
        // Mounted/partner-driven remote-player damage can temporarily report
        // the real player actor with the generic source name "Player".
        // Never let that placeholder replace a real name already learned for
        // the same actor (for example Merc -> Player). If "Player" is all we
        // know initially, a later real name is still allowed to replace it.
        if (_knownCombatantNames.TryGetValue(
                actor,
                out string? existingName) &&
            IsGenericCombatantDisplayName(friendlyName) &&
            !IsGenericCombatantDisplayName(existingName))
        {
            return;
        }
    ...
    private void TryFinalizeInactiveEncounter()
    {
        ...
        // Safety Stop freezes the current result and writes a provisional
        // history snapshot. The exact same target actor may reopen it later.
        StopEncounter(
    ...
    private bool TryResumeInactivityEncounter(
        string defender,
        double nowSeconds)
    {
        if (!_encounterComplete ||
            !_encounterResumableAfterInactivity ||
            _targetConfirmedDead ||
            _activeTargetName is null ||
            !string.Equals(
                defender,
        ...
        _encounterResumableAfterInactivity = false;

        // Keep accumulated combat time, but move the synthetic start point
        // forward so the entire Safety Stop idle gap is excluded from DPS.
        _encounterStartSeconds =
            nowSeconds - Math.Max(_finalizedDurationSeconds, 0.01);
        _lastTargetActivitySeconds = nowSeconds;

        // The provisional history entry stays as a crash-safe checkpoint while
        // combat is active. The next stop replaces it with the combined result.
    ...
    private void ProcessDeathEvent(string defender)
    {
        ...
        _targetHpActorId = defender;
        _targetCurrentHp = 0;
        _targetHpRate = 0;

        // A target can die after a Safety Stop without another accepted damage
        // line reopening the encounter. Promote the provisional snapshot to a
        // final defeated result in that case.
        if (_encounterComplete &&
            _encounterResumableAfterInactivity &&
            !_targetConfirmedDead)
        {
            _encounterSnapshotSaved = false;
            _targetConfirmedDead = true;
            RecalculateFinalDps();

            EncounterSnapshot? finalSnapshot = SaveEncounterSnapshot(
                EncounterEndReasons.TargetDefeated,
                targetConfirmedDead: true
            );

            _encounterResumableAfterInactivity = false;
            _provisionalInactivitySnapshot = null;
        ...
        string endReason)
    {
        ...
        bool isInactivityTimeout = string.Equals(
            endReason,
            EncounterEndReasons.InactivityTimeout,
            StringComparison.Ordinal
        );

        if (_encounterActive)
        {
            double encounterEndSeconds =
                isInactivityTimeout && _lastTargetActivitySeconds > 0
                    ? _lastTargetActivitySeconds
                    : _applicationClock.Elapsed.TotalSeconds;

            _finalizedDurationSeconds = Math.Max(
                encounterEndSeconds - _encounterStartSeconds,
                0.01
            );
        }
        ...
        RecalculateFinalDps();
        EncounterSnapshot? savedSnapshot =
            SaveEncounterSnapshot(endReason, targetConfirmedDead);

        if (isInactivityTimeout && !targetConfirmedDead)
        {
            _encounterResumableAfterInactivity = true;

            if (savedSnapshot is not null)
            {
                _provisionalInactivitySnapshot = savedSnapshot;
            }
        }
        else
        {
    ...
    private EncounterSnapshot? SaveEncounterSnapshot(
        string endReason,
        bool targetConfirmedDead)
    {
        ...
        if (_provisionalInactivitySnapshot is not null)
        {
            PreserveEncounterUserMetadata(
                _provisionalInactivitySnapshot,
                snapshot
            );
            _encounterHistory.Remove(_provisionalInactivitySnapshot);
        }
    ...
    private static void PreserveEncounterUserMetadata(
        EncounterSnapshot source,
        EncounterSnapshot destination)
    {
        // History supports rename/notes/pinning. When a provisional Safety Stop
        // snapshot is replaced by the combined result, preserve any user-owned
        // fields without coupling this code to the history model's exact names.
        HashSet<string> combatOwnedProperties = new(
            StringComparer.Ordinal
        )
        {
            nameof(EncounterSnapshot.TargetActorId),
            nameof(EncounterSnapshot.SessionId),
            nameof(EncounterSnapshot.TargetName),
            nameof(EncounterSnapshot.StartedAtUtc),
            nameof(EncounterSnapshot.EndedAtUtc),
            nameof(EncounterSnapshot.DurationSeconds),
            nameof(EncounterSnapshot.EndReason),
            nameof(EncounterSnapshot.TargetConfirmedDead),
            nameof(EncounterSnapshot.TotalDamage),
            nameof(EncounterSnapshot.PlayerDamage),
            nameof(EncounterSnapshot.PalDamage),
            nameof(EncounterSnapshot.TeamDps),
            nameof(EncounterSnapshot.Combatants),
            nameof(EncounterSnapshot.DamageSources),
            nameof(EncounterSnapshot.PalSkills)
        };

        foreach (var property in typeof(EncounterSnapshot).GetProperties())
        {
            if (!property.CanRead ||
                !property.CanWrite ||
                combatOwnedProperties.Contains(property.Name))
            {
                continue;
            }
            ...
            {
                // Optional history metadata must never block combat tracking.
            }
        }
    }
    ...
    private void ResetTargetHp()
    {
        _targetHpActorId = null;
        _targetCurrentHp = null;
        _targetMaxHp = null;
        _targetHpRate = null;
        _targetHpQuality = "Unavailable";
    }
    ...
    private void DisconnectTelemetry()
    {
        ...
        _pendingTelemetryText = string.Empty;
        _palOwners.Clear();
        _activePalCandidates.Clear();
        _activePalActorName = null;
        _activePalStateKnown = false;
        _palDisplayName = "No active Pal";
        _damageSources.Clear();
        _palSkillStats.Clear();
        _recentPalSkillActivations.Clear();
        _pendingDamageMetadata.Clear();
        _mountedPalSkillByRider.Clear();
    ...
    private void InspectExistingTelemetryBootstrap(string telemetryPath)
    {
        try
        {
            using FileStream stream = new(
            ...
            while ((line = reader.ReadLine()) is not null)
            {
                ...
                // The tracker deliberately begins live tailing at EOF so old
                // combat is never replayed. Identity/ownership records are
                // persistent session state, however, and must be restored
                // before the first new Pal hit arrives.
                if (fields[0].Equals(
                        "V",
                        StringComparison.OrdinalIgnoreCase) &&
                    fields.Length >= 4)
    ...
    private bool IsLuaVersionCompatible()
    {
        ...
        // v0.9.0.3 Beta retires the internal RC12.x letter tags. Keep
        // backward compatibility with RC7+ diagnostic bundles while treating
        // the new public-style semantic version as directly compatible.
        if (_luaVersion.StartsWith(
                "v0.9.0.3",
                StringComparison.OrdinalIgnoreCase))
        {
            return true;
        ...
        return _luaReleaseCandidate.GetValueOrDefault() >=
    ...
    private void RenderInterface()
    {
        ...
        bool hasLiveTargetHp =
            !_targetConfirmedDead &&
            _activeTargetName is not null &&
            _targetHpActorId is not null &&
            _targetHpActorId.Equals(
                _activeTargetName,
                StringComparison.Ordinal) &&
            _targetHpRate.HasValue;

        double targetHpPercent = hasLiveTargetHp
            ? Math.Clamp(_targetHpRate!.Value * 100.0, 0, 100)
            : 0;

        if (_targetConfirmedDead)
        {
            TargetTelemetryText.Text = "TARGET DEFEATED";
            TargetActivityBar.Value = 0;
        }
        else if (hasLiveTargetHp)
        {
            TargetActivityBar.Value = targetHpPercent;

            if (_targetCurrentHp.HasValue &&
                _targetMaxHp.HasValue &&
                _targetMaxHp.Value > 0)
            {
                string estimatePrefix = _targetHpQuality.Equals(
                    "ESTIMATED",
                    StringComparison.OrdinalIgnoreCase)
                        ? "~"
                        : string.Empty;

                TargetTelemetryText.Text =
                    $"HP {estimatePrefix}{_targetCurrentHp.Value:N0} / " +
                    $"{_targetMaxHp.Value:N0}  •  {targetHpPercent:F1}%";
            }
            else
            {
                TargetTelemetryText.Text =
                    $"LIVE HP {targetHpPercent:F1}%";
            }
        }
        else
        {
            TargetTelemetryText.Text = _activeTargetName is null
        ...
        if (_targetConfirmedDead)
        {
            CurrentHpValueText.Text = "0";
        }
        else if (hasLiveTargetHp &&
                 _targetCurrentHp.HasValue &&
                 _targetMaxHp.HasValue &&
                 _targetMaxHp.Value > 0)
        {
            string estimatePrefix = _targetHpQuality.Equals(
            ...
            CurrentHpValueText.Text =
                $"{estimatePrefix}{_targetCurrentHp.Value:N0} / " +
                $"{_targetMaxHp.Value:N0}";
        }
        else if (hasLiveTargetHp)
        {
            CurrentHpValueText.Text = $"{targetHpPercent:F1}%";
        }
        else
        {
            CurrentHpValueText.Text = "—";
...
internal sealed class MountedPalSkillContext
{
    public MountedPalSkillContext(
        string riderActorId,
        string riderDisplayName,
        string palActorId,
        string palName,
        string skillId,
        string skillName,
        double startTelemetryTimestamp)
    {
        RiderActorId = riderActorId;
        RiderDisplayName = riderDisplayName;
        PalActorId = palActorId;
        PalName = palName;
        SkillId = skillId;
        SkillName = skillName;
        StartTelemetryTimestamp = startTelemetryTimestamp;
    }

    public string RiderActorId { get; }
    public string RiderDisplayName { get; set; }
    public string PalActorId { get; }
    public string PalName { get; set; }
    public string SkillId { get; set; }
    public string SkillName { get; set; }
    public double StartTelemetryTimestamp { get; }
    public double? EndTelemetryTimestamp { get; set; }
    public string? EndPhase { get; set; }
    public bool CastCounted { get; set; }
}
